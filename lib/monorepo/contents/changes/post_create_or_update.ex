defmodule Monorepo.Contents.Changes.PostCreateOrUpdate do
  require Ash.Query

  def create_or_update_term_relationships(%{arguments: arguments}, post, _context) do
    post_id = post.id
    ...
    term_relationships =
      Enum.reduce(records, term_relationships, &([%{term_taxonomy_id: &1.term_taxonomy |> List.first() |> Map.get(:id), post_id: post_id} | &2]))

    %{status: :success, records: records} =  Ash.bulk_create!(term_relationships, Monorepo.Terms.TermRelationships, :create_term_relationships_by_relation_id, actor: %{roles: [:admin]}, return_records?: true)
