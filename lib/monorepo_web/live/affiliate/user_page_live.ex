defmodule MonorepoWeb.Affiliate.UserPageLive do
  ...
  def handle_event("save", %{"form" => form}, socket) do
    ...
    new_username = Monorepo.Utilities.Account.load_meta_value_by_meta_key(record, :username)
    ...
    if entry.done? do
      uploaded_file =
        consume_uploaded_entry(socket, entry, fn %{path: path} = _meta ->
          old_file =
            Monorepo.Utilities.Account.load_meta_value_by_meta_key(
          ...
          uploaded_file =
            case uploader do
              :avatar -> Monorepo.Utilities.Account.generate_avatar_from_entry(entry, path)
              :banner -> Monorepo.Utilities.Account.generate_banner_from_entry(entry, path)
            end
    ...
    <div>
      ...
      <div class="flex items-start gap-8">
        <div class="w-80 py-4 px-2 mt-16">
          ...
          <p class="text-sm text-gray-500 my-2 px-4">{Monorepo.Utilities.Account.load_meta_value_by_meta_key(@user, :bio)}</p>
          <div class="mb-12">
            <div>
              <div class="px-4 w-full text-sm  !text-gray-500 !outline-none break-words resize-none overflow-hidden">{Monorepo.Utilities.Account.load_meta_value_by_meta_key(@user, :description)}</div>
            </div>

            <div class="grid grid-cols-1" :if={Monorepo.Utilities.Account.load_meta_value_by_meta_key(@user, :location)}>
              <div class="col-start-1 row-start-1 block w-full rounded-md bg-white py-1.5 pl-10 pr-3 text-base text-gray-900 outline outline-1 -outline-offset-1 outline-none placeholder:text-gray-400 focus:outline focus:outline-2 focus:-outline-offset-2 focus:outline-gray-600 sm:pl-9 sm:text-sm/6">
                {Monorepo.Utilities.Account.load_meta_value_by_meta_key(@user, :location)}
              </div>
              <.icon name="hero-map-pin" class="pointer-events-none col-start-1 row-start-1 ml-3 size-5 self-center text-gray-400 sm:size-4" />
            </div>

            <div class="grid grid-cols-1" :if={Monorepo.Utilities.Account.load_meta_value_by_meta_key(@user, :website)}>
              <div class="col-start-1 row-start-1 block w-full rounded-md bg-white py-1.5 pl-10 pr-3 text-base text-gray-900 outline outline-1 -outline-offset-1 outline-none placeholder:text-gray-400 focus:outline focus:outline-2 focus:-outline-offset-2 focus:outline-gray-600 sm:pl-9 sm:text-sm/6">
                {Monorepo.Utilities.Account.load_meta_value_by_meta_key(@user, :website)}
              </div>
            ...
            <div class="grid grid-cols-1" :if={Monorepo.Utilities.Account.load_meta_value_by_meta_key(@user, :twitter)}>
              <div class="col-start-1 row-start-1 block w-full rounded-md bg-white py-1.5 pl-10 pr-3 text-base text-gray-900 outline outline-1 -outline-offset-1 outline-none placeholder:text-gray-400 focus:outline focus:outline-2 focus:-outline-offset-2 focus:outline-gray-600 sm:pl-9 sm:text-sm/6">
                {Monorepo.Utilities.Account.load_meta_value_by_meta_key(@user, :twitter)}
              </div>
            ...
            <div class="grid grid-cols-1" :if={Monorepo.Utilities.Account.load_meta_value_by_meta_key(@user, :facebook)}>
              <div class="col-start-1 row-start-1 block w-full rounded-md bg-white py-1.5 pl-10 pr-3 text-base text-gray-900 outline outline-1 -outline-offset-1 outline-none placeholder:text-gray-400 focus:outline focus:outline-2 focus:-outline-offset-2 focus:outline-gray-600 sm:pl-9 sm:text-sm/6">
                {Monorepo.Utilities.Account.load_meta_value_by_meta_key(@user, :facebook)}
              </div>
            ...
            <div class="grid grid-cols-1" :if={Monorepo.Utilities.Account.load_meta_value_by_meta_key(@user, :instagram)}>
              <div class="col-start-1 row-start-1 block w-full rounded-md bg-white py-1.5 pl-10 pr-3 text-base text-gray-900 outline outline-1 -outline-offset-1 outline-none placeholder:text-gray-400 focus:outline focus:outline-2 focus:-outline-offset-2 focus:outline-gray-600 sm:pl-9 sm:text-sm/6">
                {Monorepo.Utilities.Account.load_meta_value_by_meta_key(@user, :instagram)}
              </div>
    ...
    >
      <.form for={@form} phx-change="form-changed" phx-submit="save">
        <div class="relative lg:h-[120px] bg-primary">
          <div class="w-full h-full overflow-hidden">
            <img :if={Monorepo.Utilities.Account.load_meta_value_by_meta_key(@user, :banner)["xl"]} class="w-full h-full object-cover overflow-hidden" src={Monorepo.Utilities.Account.load_meta_value_by_meta_key(@user, :banner)["xl"]} />
          ...
          <div class="mx-2 flex justify-between items-end -mt-10">
            <div class="relative size-20">
              <img :if={Monorepo.Utilities.Account.load_meta_value_by_meta_key(@user, :avatar)} class="inline-block size-20 rounded-full" src={Monorepo.Utilities.Account.load_meta_value_by_meta_key(@user, :avatar)["128"]} alt="">
              <span :if={!Monorepo.Utilities.Account.load_meta_value_by_meta_key(@user, :avatar)} class="inline-flex size-20 items-center justify-center rounded-full bg-primary border-2 border-white">
                <span class="font-medium text-white uppercase text-4xl">{Monorepo.Utilities.Account.load_meta_value_by_meta_key(@user, :name) |> String.slice(0, 1)}</span>
              </span>
        ...
        <div class="px-4 mt-16 space-y-6">
          <.user_meta_input
            ...
            field={%FormField{
              ...
              value: Monorepo.Utilities.Account.load_meta_value_by_meta_key(@user, f),
              errors: [],
              field: :user_meta,
              form: @form
            }}
  ...
  defp is_modified_username?(user) do
    username = Monorepo.Utilities.Account.load_meta_value_by_meta_key(user, :username)
    name = Monorepo.Utilities.Account.load_meta_value_by_meta_key(user, :name)
