defmodule MonorepoWeb.AdminPostLive.NavBar do
  use MonorepoWeb.Admin, :live_component

  def render(assigns) do
    ~H"""
    <div id={@id} class="flex h-16 shrink-0 items-center gap-x-4 border-b border-gray-200 bg-white px-4 sm:gap-x-6 sm:px-6 lg:px-8">
      ...
      <div class="flex flex-1 gap-x-4 self-stretch items-center lg:gap-x-6">
        <.link class={["text-sm ml-auto pointer-events-none opacity-50"]} data-id="save-draft"
          phx-click={
        ...
        <.link class={["text-sm flex items-center gap-x-1 pointer-events-none opacity-50"]} data-confirm="Are you sure you want to clear the editor?" data-id="clear-editor" phx-click={JS.dispatch("clear_editor")}>
