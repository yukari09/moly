defmodule MonorepoWeb.AdminPageLive.Create do
  use MonorepoWeb.Admin, :live_view

  def mount(%{"preview_id" => preview_id, "layout" => layout}, _session, socket) do
    Phoenix.PubSub.subscribe(Monorepo.PubSub, "#{pubsub_channel(preview_id)}")
    layout = if layout == "false", do: false, else: {MonorepoWeb.Layouts, :app}
    preview_content = get_or_put_preview_content_cache(preview_id)
    {:ok, socket, layout: layout, temporary_assigns: [preview_content: preview_content, preview_id: preview_id]}
  end

  def mount(_params, _session, socket) do
    editor_id  = Monorepo.Helper.generate_random_id()
    preview_id = Monorepo.Helper.generate_random_id()
    textarea_id = Monorepo.Helper.generate_random_id()
  ...
  defp get_or_put_preview_content_cache(preview_id, preview_content_value \\ nil) do
    ...
    if preview_content_value, do: Cachex.put!(:cache, key, preview_content_value, expire: :timer.minutes(15)), else: Cachex.get!(:cache, key)
  end

  defp pubsub_channel(preview_id), do: "#{MonorepoWeb.Admin.topic(:page)}:#{preview_id}"

  defp broadcast(socket) do
    pubsub_channel =  pubsub_channel(socket.assigns.preview_id)
    Phoenix.PubSub.broadcast(Monorepo.PubSub, pubsub_channel, %{new_link: live_view_url(socket)})
    socket
  end
  ...
  defp params_to_form_data(socket, params) do
    ...
    if invaild_form_data do
      ...
      form =
        AshPhoenix.Form.for_create(Monorepo.Contents.Post, :create_post, forms: [auto?: true], actor: socket.assigns.current_user)
        |> to_form()
