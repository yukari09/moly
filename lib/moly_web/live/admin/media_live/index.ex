defmodule MolyWeb.AdminMediaLive.Index do
  ...
  def mount(params, _session, socket) do
    ...
    [socket, opts] =
      if Map.has_key?(params, "modal") do
        [assign(socket, :modal, true), [layout: {MolyWeb.Layouts, :admin_modal}]]
      else
    ...
    socket =
      case upload_entry_information(entry, path) do
        ...
        %{mime_type: mime_type, file: file, filename: filename, filesize: filesize} = meta_data ->
          ...
          {:ok, media} = Moly.Contents.create_media(attrs, actor: socket.assigns.current_user)
