defmodule MolyWeb.AdminPostLive.SideBar do
    ...
    >
      <div class="h-full overflow-y-scroll bg-white flex flex-col px-2 sm:px-4 space-y-4">
        ...
        <div class="space-y-5">
          <.live_component
            id="feature-image"
            module={MolyWeb.AdminPostLive.FormField.Thumbnail}
            form={@form}
            current_user={@current_user}
          />

          <div :if={@form.data} class="text-gray-600 text-sm mx-2">
            Last edited {Timex.Format.DateTime.Formatters.Relative.format!(
          ...
          <ul class="space-y-4 mx-2 text-sm">
            ...
            <li>
              <.live_component
                id="form-field-post-date"
                module={MolyWeb.AdminPostLive.FormField.PostDate}
                form={@form}
              />
            </li>
            <li>
            ...
            <li class="flex items-center">
              ...
              <.toggle_switch
                ...
                enabled={true}
                on_change={
                  JS.toggle_attribute({"value", "1", "0"}, to: "[data-id='meta-value-comments-open']")
                }
              />
              <input
                data-id="meta-key-comments-open"
                name={"#{@form[:post_meta].name}[1][meta_key]"}
                value={:comments_open}
                type="hidden"
              />
              <input
                data-id="meta-value-comments-open"
                name={"#{@form[:post_meta].name}[1][meta_value]"}
                value="1"
                type="hidden"
              />
            </li>
        ...
        <ul role="list" class="space-y-2">
          ...
          <li>
            <.live_component
              ...
              module={MolyWeb.AdminPostLive.FormField.PostCategories}
              current_user={@current_user}
              form={@form}
            />
          </li>
              ...
              >
                <div id="tagify-input-container" class="mt-2" phx-update="ignore">
                  <input
                    ...
                    id="tagify-input"
                    phx-hook="TagsTagify"
                    data-target-name={"#{@form[:tags].name}"}
                    data-target-container="#tagify-input-target"
                    value={(@form.data && Enum.map_join(@form.data.post_tags, ",", & &1.name)) || []}
                  />
                </div>
                <div
                  class="mb-12 text-xs text-gray-500 py-2"
                  id="tagify-input-target"
                  phx-update="ignore"
                >
                  <input
                    :for={{tag, i} <- Enum.with_index((@form.data && @form.data.post_tags) || [])}
                    name={"#{@form[:tags].name}[#{i}][name]"}
                    value={tag.name}
                    data-value={tag.name}
                    type="hidden"
                  />
