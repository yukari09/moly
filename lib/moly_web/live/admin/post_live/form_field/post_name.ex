defmodule MolyWeb.AdminPostLive.FormField.PostName do
    ...
    >
      ...
      <.dropdown id={@slug_dropdown_id} data-id={@slug_dropdown_id} class="flex-1">
        <:button_slot class="text-gray-600 !p-0 !m-0">
          <span data-id="post-slug-label" class="hover:underline">
            {@form[:post_name].value || @post_slug}
          </span>
        </:button_slot>
          ...
          <div class="space-y-4 py-2">
            ...
            <div>
              <span class="font-medium text-sm/6">
                Permanent link:
                <span class="font-normal" data-id="post-slug-text">
                  {"#{@host}#{@form[:post_name].value || @post_slug}"}
                </span>
              </span>
            </div>
          </div>
      ...
      <input
        data-id="post-post-name-oi"
        name={@form[:post_name].name}
        value={@form[:post_name].value || @post_slug}
        type="hidden"
        class="hidden"
      />
      <.input
