defmodule MolyWeb.AdminPostLive.FormField.PostDate do
  ...
  def render(assigns) do
    ...
    <div id="post-date" class="flex items-center" phx-update="ignore">
      ...
      <label
        ...
        for="post-date-input-picker"
      >
        <span
          data-id="post-date-immediately"
          class={[(!@form[:post_date].value && "flex items-center") || "hidden"]}
        >
          Immediately&nbsp;<Lucideicons.calendar class="w-4 h-4 text-gray-500" />
        </span>
        <span data-id="post-date-schedule" class={[!@form[:post_date].value && "hidden"]}>
          {@form[:post_date].value}
        </span>
      </label>
      <.input
