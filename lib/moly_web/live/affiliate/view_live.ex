defmodule MolyWeb.Affiliate.ViewLive do
  ...
  def mount(_params, _session, socket) do
    ...
    industry_category =
      Moly.Utilities.cache_get_or_put(
        :viewliview_industries,
        fn ->
          Moly.Terms.read_by_term_slug!("industries", actor: %{roles: [:user]})
          |> List.first()
        end,
        :timer.hours(2)
      )
  ...
  def handle_params(%{"post_name" => post_name}, _uri, socket) do
    ...
    post =
      Moly.Utilities.cache_get_or_put(
        ":liveview_post_#{post_name}",
        fn ->
          Ash.Query.for_read(Moly.Contents.Post, :read)
    ...
    current_user_bookmark =
      if socket.assigns.current_user do
        Ash.Query.for_read(Moly.Accounts.UserPostAction, :read)
        |> Ash.Query.filter(
          user_id == ^socket.assigns.current_user.id and post_id == ^post.id and
            action == :bookmark
        )
        |> Ash.read_first(opts)
        |> case do
  ...
  defp html_mata(%{assigns: %{post: post}} = socket) do
    media_url =
      Moly.Utilities.Affiliate.affiliate_media_feature_src_with_specific_sizes(post, [
        "xlarge",
        "large",
        "medium"
      ])
