defmodule MolyWeb.Affinew.Components do
  ...
  def commission_options, do: [{"USD", "$"}, {"EUR", "€"}, {"%", "%"}]
  ...
  def commission_option(label), do: commission_options() |> Map.new(&{elem(&1, 0), elem(&1, 1)}) |> Map.get(label)
  ...
  def card(assigns) do
    ...
    <figure :if={featrue_image_src(@post)} class="aspect-video overflow-hidden !block relative text-base-content rounded-t-lg">
      ...
      <div class="card-body">
        <h2 class="text-lg font-bold flex items-top gap-2 mb-1" >
  ...
  def filter_dropdown(assigns) do
    ...
    ~H"""
    <button class={["btn btn-xs sm:btn-sm md:btn-md bg-white text-black border-[#e5e5e5] flex items-center rounded-md", @class]} popovertarget={@id} style={"anchor-name:--anchor-#{@id}"}>
      {@value && Enum.find(@options, &(elem(&1, 0) == @value)) |> elem(1) || @label}<Lucideicons.chevron_down class="size-4"/>
    </button>
    <ul class="dropdown menu mt-1 w-auto rounded-box bg-white border border-base-300 max-h-[200px] overflow:scroll-x shadow-lg"
      popover id={@id}
      style={"position-anchor:--anchor-#{@id}"}
    >
      <li :for={{value, label} <- @options}>
        <.link class="font-medium" patch={Links.programs(Map.put(@params, @name, value))}>{label}</.link>
      </li>
    </ul>
    """
  end
  ...
  defp commission_text(%{data: c} = assigns) do
    ...
    ~H"""
    <span :if={@commission_type == "bounty"}>&nbsp;<span class="font-semibold text-[#ff5000]">{commission_option(@commission_unit) || @commission_unit}{Moly.Helper.format_to_int(@commission_amount,1)}</span> {Map.get(@data, :commission_notes) || "Fixed Bounty"}</span>
    <span :if={@commission_type == "revenue_share"}>&nbsp;<span class="font-semibold text-[#ff5000]">{Moly.Helper.format_to_int(@commission_amount,0)}%</span>  {Map.get(@data, :commission_notes) || "Revenue Share"}</span>
    <span :if={@commission_type == "hybrid"}>&nbsp;<span class="font-semibold text-[#ff5000]">{Moly.Helper.format_to_int(@commission_amount,1)}{Moly.Helper.format_to_int(@commission_amount,1)}</span>  {Map.get(@data, :commission_notes)}</span>
    """
  end
