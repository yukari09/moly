defmodule MolyWeb.Affinew.Components do
  ...
  def view_title(assigns) do
    ~H"""
    <div>
      <h1 class="text-2xl md:text-3xl lg:text-4xl font-semibold text-primary">
        {Moly.Helper.get_in_from_keys(@post, [:source, "post_title"])}
      </h1>
      <div class="flex flex-col md:flex-row md:items-center gap-2 mt-6 text-base-content/60">
        ...
        <div class="flex items-center gap-2 mt-2 md:mt-0">
          ...
          <.link
            ...
            class="text-sm flex items-center gap-1 link link-hover"
          >
          ...
          <.link class="text-sm flex items-center gap-1 link link-hover" phx-click={@bookmark_event}>
            <span :if={@bookmark_event in ["require_login", "bookmark_post"]}>
              <Lucideicons.book_marked class="size-4 inline" /> Save
            </span>
            <span :if={@bookmark_event in ["unbookmark_post"]}>
              <Lucideicons.bookmark_check class="size-4 inline" /> Saved
            </span>
          </.link>
          <.link
            href={Moly.Helper.get_in_from_keys(@post, [:source, "affiliate_program_link"])}
            rel="nofollow"
            target="_blank"
            class="text-sm flex items-center gap-1 link link-hover"
          >
    ...
    <!--intro-->
    <p class="mt-4 md:mt-8 line-clamp-3">
      {Moly.Helper.get_in_from_keys(@post, [:source, "post_excerpt"])}
    </p>
    <ul class="list bg-[rgb(255,253,246)] rounded-box mt-4 md:mt-12">
      ...
      <li
        :for={
          {commission, i} <-
            Moly.Helper.get_in_from_keys(@post, [:source, "commission"]) |> Enum.with_index()
        }
        class="list-row"
      >
        <div class="hidden text-2xl md:text-4xl font-thin opacity-30 tabular-nums md:flex flex-col justify-center">
          0{i + 1}
        </div>
        <div class="flex flex-col justify-center lg:max-w-16 uppercase font-semibold opacity-60 text-xs">
          {Map.get(commission, "commission_type") |> commission_type_option_label()}
        </div>
        <div class="flex flex-col justify-center lg:min-w-28">
          <div
            :if={Map.get(commission, "commission_unit") != "%"}
            class="text-xl/6 md:text-2xl/6 text-[#ff5000]"
          >
            {Map.get(commission, "commission_unit") |> commission_unit_option()}{Map.get(
              commission,
              "commission_amount"
            )
            |> Moly.Helper.format_to_int(2)}
          </div>
          <div
            :if={Map.get(commission, "commission_unit") == "%"}
            class="text-xl/6 md:text-2xl/6 text-[#ff5000] mt-1"
          >
    ...
    <dialog id="share_modal" class="modal">
      <div class="modal-box w-auto relative">
        ...
        <div class="grid grid-cols-4 gap-4 py-4 mt-4 rounded-box">
          <div
            id="share-twitter"
            class="text-center space-y-1 cursor-pointer"
            data-share="twitter"
            phx-hook="ShareHook"
          >
            <svg
            ...
            >
              <rect width="512" height="509.64" rx="115.61" ry="115.61" /><path
                fill="#fff"
                fill-rule="nonzero"
          ...
          >
            <svg
              ...
              viewBox="0 0 512 509.64"
            >
              <rect fill="#0866FF" width="512" height="509.64" rx="115.612" ry="115.612" /><path
                fill="#fff"
          ...
          >
            <svg
              ...
              viewBox="0 0 512 512"
            >
              <path d="M105 0h302c57.75 0 105 47.25 105 105v302c0 57.75-47.25 105-105 105H105C47.25 512 0 464.75 0 407V105C0 47.25 47.25 0 105 0z" /><path
                fill="#fff"
                fill-rule="nonzero"
          ...
          >
            ...
            >
              <path
                d="M512 128v256c0 70.646-57.355 128-128 128H128C57.354 512 0 454.646 0 384V128C0 57.354 57.354 0 128 0h256c70.645 0 128 57.354 128 128z"
                fill="#ff4500"
              />
              <g transform="translate(1.249 1.608) scale(1.99024)">
                <circle cx="200.6" cy="123.7" r="29.9" fill="url(#prefix___Radial1)" /><circle
                  cx="55.4"
                  cy="123.7"
                  r="29.9"
                  fill="url(#prefix___Radial2)"
                /><ellipse cx="128.1" cy="149.3" rx="85.3" ry="64" fill="url(#prefix___Radial3)" /><path
                ...
                /><path
                  ...
                  fill-rule="nonzero"
                /><circle cx="174.8" cy="55.5" r="21.2" fill="url(#prefix___Radial7)" /><path
                ...
                /><path
                  ...
                  fill="#ff6101"
                  fill-rule="nonzero"
                /><ellipse cx="94.4" cy="134.8" rx="3.3" ry="3.6" fill="#ffc49c" /><ellipse
                  cx="173.3"
                  cy="134.8"
                  rx="3.3"
                  ry="3.6"
              ...
              <defs>
                ...
                >
                  <stop offset="0" stop-color="#feffff" /><stop offset=".4" stop-color="#feffff" /><stop
                    offset=".51"
                    stop-color="#f9fcfc"
                  /><stop offset=".62" stop-color="#edf3f5" /><stop offset=".7" stop-color="#dee9ec" /><stop
                    offset=".72"
                    stop-color="#d8e4e8"
                  /><stop offset=".76" stop-color="#ccd8df" /><stop offset=".8" stop-color="#c8d5dd" /><stop
                    offset=".83"
                    stop-color="#ccd6de"
                  /><stop offset=".85" stop-color="#d8dbe2" /><stop offset=".88" stop-color="#ede3e9" /><stop
                    offset=".9"
                    stop-color="#ffebef"
                ...
                >
                  <stop offset="0" stop-color="#f60" /><stop offset=".5" stop-color="#ff4500" /><stop
                    offset=".7"
                    stop-color="#fc4301"
                  /><stop offset=".82" stop-color="#f43f07" /><stop offset=".92" stop-color="#e53812" /><stop
                    offset="1"
                    stop-color="#d4301f"
                  />
                </radialGradient>
                ...
                >
                  <stop offset="0" stop-color="#172e35" /><stop offset=".29" stop-color="#0e1c21" /><stop
                    offset=".73"
                    stop-color="#030708"
                  /><stop offset="1" />
                </radialGradient>
                ...
                >
                  <stop offset="0" stop-color="#7a9299" /><stop offset=".48" stop-color="#7a9299" /><stop
                    offset=".67"
                    stop-color="#172e35"
                  /><stop offset=".75" /><stop offset=".82" stop-color="#172e35" /><stop
                    offset="1"
                    stop-color="#172e35"
                  />
                </radialGradient>
        ...
        <button
          onclick="share_modal.close()"
          class="btn btn-sm btn-circle mt-2 mr-2 absolute right-0 top-0"
        >
  ...
  def view_base_info(assigns) do
    ...
    <div class="grid grid-cols-2 gap-4 px-4 py-6 bg-green-light/10">
      ...
      <div :if={Moly.Helper.get_in_from_keys(@post, [:source, "min_payout_threshold"]) > 0}>
        <span class="text-lg text-primary font-medium">
          {Moly.Helper.get_in_from_keys(@post, [:source, "min_payout_threshold"])}
        </span>
        <span class="text-xs text-base-content/60">
          {Moly.Helper.get_in_from_keys(@post, [:source, "currency"])}
        </span>
      </div>
      <div :if={Moly.Helper.get_in_from_keys(@post, [:source, "min_payout_threshold"]) == 0}>
        Not specified
      ...
      <div class="text-sm font-medium">
        {Moly.Helper.get_in_from_keys(@post, [:source, "payment_method"]) |> Enum.join(",")}
      </div>
      ...
      <div class="text-sm font-medium uppercase">
        {Moly.Helper.get_in_from_keys(@post, [:source, "payment_cycle"])}
      </div>
      <div class="text-sm font-medium">Applicable Region</div>
      <div class="text-sm font-medium">
        {Moly.Helper.get_in_from_keys(@post, [:source, "region"])}
      </div>
    </div>
    """
  end
  ...
  def view_description(assigns) do
    ...
    <div class="prose max-w-none text-base-content/70">
      {Moly.Helper.get_in_from_keys(@post, [:source, "post_content"]) |> to_safe_html}
    </div>
    """
  end
  ...
  def view_signup_requirements(assigns) do
    ...
    <div class="prose max-w-none  text-base-content/70">
      {Moly.Helper.get_in_from_keys(@post, [:source, "affiliate_signup_requirements"]) |> to_safe_html}
    </div>
    """
  end
  ...
  def view_category(assigns) do
    ...
    <div class="px-2 lg:px-8 py-4 rounded-box bg-base-100/50">
      ...
      <p class="mt-4 text-sm text-base-content/80">
        <.link navigate={
          Moly.Helper.get_in_from_keys(@post, [:source, "affiliate_category", 0, "slug"])
          |> Links.term()
        }>
          {Moly.Helper.get_in_from_keys(@post, [:source, "affiliate_category", 0, "name"])}
        </.link>
    ...
    <div class="px-2 lg:px-8 py-4 rounded-box bg-base-100/50">
      ...
      <p class="mt-4 text-sm text-base-content/80 flex items-center gap-4 flex-wrap">
        <.link
          :for={
            %{"name" => name, "slug" => slug} <-
              Moly.Helper.get_in_from_keys(@post, [:source, "affiliate_tag"])
          }
